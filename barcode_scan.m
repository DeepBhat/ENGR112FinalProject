function [material_type_code,material_type_string,t1,t2] = barcodScan()
...
myev3 = legoev3('USB');
mycolorsensor = colorSensor(myev3,1); %create a color sensor connection
mymotor = motor(myev3,'A'); %create a motor connection
%===========================================================================
...
for i=1:8
    while(1)
        start(mymotor);
        if readRotation(mymotor)>=180/(pi*1.075)
            break
        else
            pause(realmin);
        end
    ...
    elseif(color=="white")
        ...
        error('No black or white color detected'); %if neither black nor white color detected
    end
